...
while True:
    ...
    def hint_gen(guess_word, c):  # creating the word with hints
        for i in range(
                0, len(guess_word), 3):  # i will be used as index numbers
            # from 0 to lenght 0-9 with step size of three to get every third
            # letter
    ...
    def detector(guess_word, guess):
        ...
        for i in guess_word:
            if i == guess:
                # adding the count which is representing the index to the end
                # list a
                a.append(count)
            count += 1
    ...
    def modifier(c, guess_word="roadrunner", guess="r"):
        ...
        c = list(c)
        r = ""
    # will be modified c where the dashes are replaced back with the correct
    # letter
        for i in a:
            c[i] = guess
    ...
    while True:
        ...
        if mode == "1":  # if 1 is selected a word with hints will be generated
            ...
            print(
                ...
                "For hardcore game mode you have no letters and have to" +
                "guess with no hints")
            continue
        ...
        while tries > 0 and c != correct_ans:  # tries are greated than 0 c = 0
            flag = False  # indication of whether the user the guessed letter

            if c != correct_ans:  # c is not the correct answer print try guess
                print("\nTry to guess this word", c, "")
                try:
                    guess = input("\nGuess a letter: ")
                    # letter is 65-90 for uppercase alphabets and 97-122
                ...
                if not flag:  # if guess if false
                    # guess is false it will add letter to already used letters
                    # on screen
